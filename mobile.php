<title> Mobile Source | The Hub </title>
<?php
require ('core.inc.php');
include ('connect.php');
$sql = "CREATE TABLE mobile_source ( `id` INT(11) NOT NULL AUTO_INCREMENT , `session` VARCHAR(50) NOT NULL , `text1` TEXT(250) NOT NULL, `pic` VARCHAR(50) NOT NULL, `name` VARCHAR(25) NOT NULL , PRIMARY KEY (`id`))";
$run = mysqli_query($link, $sql);
$sqlo ="ALTER TABLE mobile_source ADD file_name VARCHAR(40) NOT NULL AFTER `name`";
          ...
          //moving uploaded files to directory
            if (isset($_POST['text'])&&!empty('text')) {
              ...
              @$row = mysqli_fetch_array($result);
                ...
                $query = "INSERT INTO mobile_source(id, session, text1, pic, name, file_name) VALUES ('', '".$session."', '".$text."', '".$pic."', '".$name."', '".$basename."')";
           if ($query_run = mysqli_query($link, $query)) {
             echo "<div class=\"alert alert-success alert-dismissable\">
           <button type=\"button\" class=\"close\" data-dismiss=\"alert\" aria-hidden=\"true\">&times;</button>
           The file ".basename($_FILES["fileToUpload"]["name"]). " has been uploaded. Successfully!
           </div> <p>Click <a href=\"mobile.php\">here</a> to return ";
           die();
            }
}
}
...
$sql= "SELECT * FROM mobile_source ORDER BY id DESC";
$result = mysqli_query($link, $sql);
while (@$row = mysqli_fetch_array($result)) {
$img = $row['pic'];
$dirname = 'profile/'.$img;
