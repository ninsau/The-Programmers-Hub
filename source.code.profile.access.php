...
Browse through available source code files below !
  ...
  while ($files = readdir($handle)) {
    if ($files != '.' && $files != '..') {
      $sql= "SELECT * FROM web_source WHERE session = '".$_SESSION['profile']."'";
...
if ($handle = opendir($target_dir)) {
  ...
  while ($files = readdir($handle)) {
    if ($files != '.' && $files != '..') {
      $sql= "SELECT * FROM system_source WHERE session = '".$_SESSION['profile']."'";
      $result = mysqli_query($link, $sql);
      while (@$row = mysqli_fetch_array($result)) {
      $img = $row['pic'];
      $dirname = 'profile/'.$img;
...
if ($handle = opendir($target_dir)) {
  ...
  while ($files = readdir($handle)) {
    if ($files != '.' && $files != '..') {
      $sql= "SELECT * FROM mobile_source WHERE session = '".$_SESSION['profile']."'";
      $result = mysqli_query($link, $sql);
      while (@$row = mysqli_fetch_array($result)) {
      $img = $row['pic'];
      $dirname = 'profile/'.$img;
